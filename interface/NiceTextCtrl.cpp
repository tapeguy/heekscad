...
double CDoubleCtrl::GetValueAsDouble()
{
	double value = 0.0;
...
void CDoubleCtrl::SetValueFromDouble(double value)
{
	wxTextCtrl::SetValue(DoubleToString(value));
}

CLengthCtrl::CLengthCtrl(wxWindow* parent, wxWindowID id)
:CDoubleCtrl(parent, id, Length::Conversion ( wxGetApp().GetViewUnits(), UnitTypeMillimeter ) )
{
}
